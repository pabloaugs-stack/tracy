...
Sprint 7 (financeiro completo) — desenho fechado em 5 fatias (ver `tracy-handoff.md`). **Fatia 1 (Lançamentos)**, **Fatia 2 (estoque por lote/FIFO + custo)**, **Fatia 3 (comissão automática + comissões a pagar)** e **Fatia 4 (Caixa — extrato com saldo acumulado + número de comanda)** concluídas (Fatias 2, 3 e 4 aguardando validação visual do Pablo). Fatia 5 (lucro real/DRE) na fila; vai consumir `fee_amount` + o COGS por lote da Fatia 2 + a comissão gerada da Fatia 3 para o lucro líquido.

Último trabalho fechado: **Sprint 7 / Fatia 4 — Caixa (extrato com saldo acumulado) + número de comanda**. Implementado, `test:cashflow` 22/22 (14 cenários), regressões verdes (commission 14/14, inventory-lots 25/25, financial-entries 34/34, payment-split 20/20, comanda-completa 29/29), type-check + build OK.
...
## Constantes arquiteturais — Comissão automática (Sprint 7 / Fatia 3)
- **Resolução PURA em `lib/commission/resolve.ts` (`resolveCommissionPercent`).** Sem IO, testável isolada. Regra: papel real na comanda × tipo de comissão da profissional. Papel: `auxiliar`→`como_auxiliar`; `trancista` sozinha (única profissional da comanda)→`sozinha`; `trancista` com outra profissional→`com_auxiliar`. `isSolo` é calculado no Server Action (contagem de profissionais == 1) e passado — **não** rebuscar dentro da função. **Nota:** o determinante de `isSolo` é "trancista é a única profissional" (bate com os cenários de teste 4/5); a redação de "outra trancista" nas decisões travadas era ambígua — a implementação segue os testes 14/14.
- **`commission_type` (users, default `'categoria'`):** `nao_comissiona`→0 · `categoria`→`commission_default_trancista/auxiliar` do serviço (comportamento legado preservado) · `simples`→`commission_simple_percent` · `avancado`→`commission_solo/with_aux/as_aux_percent` conforme papel. Default `categoria` não quebra nada.
- **`resolveAndSaveCommissions` (em `app/actions/commission.ts`) é chamada DENTRO do fechamento** (`closeAppointmentAction` → helper `accrueCommissions`), nunca como action de UI. Usa **admin client** (o fechamento já usa; roda mesmo quando quem fecha não tem Financeiro — dona solo/trancista alocada). **Falha não reverte o fechamento** (comissão é accrual; loga e segue). Grava `appointments.discount_affects_commission` e faz **UPSERT** em `commission_entries` pelo índice único `(appointment_id, professional_id) WHERE active`.
- **Base de comissão de SERVIÇO:** `discount_affects_commission=false` (default) → `total_price` (valor cheio); `true` → `computeFinalTotal` **sem produtos** (serviço com desconto). Produtos NUNCA entram na base de serviço. Comissão de produto = `commission_percent_snapshot` × subtotal da linha onde `sold_by_user_id` = a profissional (só quando `product_commission_enabled`).
- **Override de comissão por comanda tem GATE:** `commission_override` (em `appointment_professionals`) só é usado se `canUseOverride` = dono/gerente OU `can_edit_commission`. Na UI (`ComandaForm`) o input visível do override só aparece com esse gate; o input OCULTO permanece sempre (preserva o valor na edição por quem não pode editar). Flag `users.can_edit_commission` (default false; dono/gerente implícitos) no `PermissionsModal` e na criação (só para papéis ≠ gerente).
- **Reabertura NÃO apaga `commission_entries`** (diferente do `final`): persistem para o refechamento marcar `has_divergence=true` quando o valor muda numa entrada já `pago`. `reopenAppointmentAction` continua intocado quanto a comissão.
- **`commission_entries` e `commission_payments`:** RLS `ce_select` = financeiro OU `professional_id = auth.uid()` (a profissional vê a própria); `ce_insert` = só salão (para o accrual rodar sob qualquer usuário do salão); `ce_update`/`cp_*` = financeiro. Sem DELETE (soft delete via `active`). FKs para `appointments`/`users`/`salons` são RESTRICT. `commission_payments` agrupa N entradas de UMA profissional (pagamento com seleção livre). `registerCommissionPaymentAction` valida que as entradas pertencem ao salão+profissional e estão pendentes, cria o pagamento e marca `status='pago'` + `commission_payment_id`.
- **`salon_settings.commission_cycle`** (`semanal|quinzenal|mensal|livre`, default `livre`): só **pré-filtra** a tela Comissões a pagar (`/admin/financeiro/comissoes`) — não força periodicidade nem afeta o accrual.

## Constantes arquiteturais — Caixa + número de comanda (Sprint 7 / Fatia 4)
- **`appointment_number` é gerado por trigger, NUNCA setar manualmente.** Trigger `assign_appointment_number` (BEFORE INSERT, SECURITY DEFINER) grava `MAX(appointment_number)+1` **por salão** (não global). `insertAppointment` não precisa fazer nada — o trigger cuida. Comanda legada (pré-trigger) tem número **NULL** → exibida como **"#—"** via `formatAppointmentNumber` (`lib/appointments/format.ts`; pad de 3 dígitos, null→"#—"). Índice único parcial `(salon_id, appointment_number) WHERE appointment_number IS NOT NULL`.
- **O Caixa é CALCULADO, não é tabela.** `running_balance` é acumulado no **servidor** após ordenação por (date, created_at) — **nunca armazenado no banco**. É o saldo REAL do caixa naquele ponto (inclui movimentações anteriores ao início do período exibido, como extrato bancário). Regime de CAIXA (por `paid_at`), não competência.
- **Lógica pura separada da IO.** `lib/cashflow/compute.ts` = PURA (assemble/running-balance/summarize/preview + tipos + row-shapes agnósticas de IO), testável isolada. `lib/queries/cashflow.ts` = camada de IO fina (fetch das 4 fontes → delega à pura). `test:cashflow` alimenta os mesmos dados crus reais (via admin, mesmos filtros) nas funções puras. Não duplicar a matemática do saldo fora de `compute.ts`.
- **`opening_balance_date` NULL = incluir toda a história** (sem filtro de data de início); preenchida = o extrato começa nessa data (fetch aplica `.gte('paid_at', date)`). `opening_balance` e `opening_balance_date` em `salon_settings`. Alterar recalcula todo o extrato.
- **Fórmula:** `opening_balance + Σ appointment_payments.amount (active, por paid_at) + Σ aportes − Σ despesas/retiradas pagas − Σ commission_payments − Σ inventory_purchase_payments pagas`. O `amount` do pagamento já é o valor resolvido — não reaplicar `computeFinalTotal`. `fee_amount` de cartão **não** entra no Caixa (é custo; fica pro Lucro real / Fatia 5).
- **Parcelas de compra pendentes entram SÓ na previsão, nunca no saldo.** O Caixa lê `inventory_purchase_payments` com `status='pago'`; **nunca** `inventory_purchases` diretamente. **Compra legada sem parcelas é ignorada como saída** (não criar parcela retroativa automática). `inventory_purchases.status` (default `'pago'`) é sincronizado com as parcelas quando existem (`syncPurchaseStatus`), mas o Caixa não o consome.
- **`updateOpeningBalanceAction` grava via ADMIN client** (gate `canViewFinancial`): a RLS de UPDATE em `salon_settings` é dono/gerente-only, mas o gate do módulo Caixa é `can_view_financial` — o gate de código é o guarda real. A seção Caixa em `/admin/configuracoes` só renderiza quando `canViewFinancial(profile)` (a página é dono/gerente, mas um gerente sem a flag não veria a seção, alinhando com o gate da action). `createPurchasePaymentsAction`/`markPurchasePaymentPaidAction` = gate dono/gerente, client normal (RLS de `inventory_purchase_payments` é o guarda). Σ das parcelas DEVE bater com `total_cost` (tolerância R$ 0,01) — validado na action, não no banco.
