...
Sprint 7 (financeiro completo) — desenho fechado em 5 fatias (ver `tracy-handoff.md`). **Fatia 1 (Lançamentos)**, **Fatia 2 (estoque por lote/FIFO + custo)** e **Fatia 3 (comissão automática + comissões a pagar)** concluídas (Fatias 2 e 3 aguardando validação visual do Pablo). Fatias 4–5 (caixa, lucro real/DRE) na fila; a 5 vai consumir `fee_amount` + o COGS por lote da Fatia 2 + a comissão gerada da Fatia 3 para o lucro líquido.

Último trabalho fechado: **Sprint 7 / Fatia 3 — Comissão automática + Comissões a pagar**. Implementado, `test:commission` 14/14, regressões verdes (close-as-allocated 13/13, payment-split 20/20, financial-entries 34/34), type-check + build OK. A pausa de infra (GitHub + Vercel) já foi resolvida (repo conectado, deploy automático).
